/**
 * 客户端数据加载 — 通过 fetch('/data/prompts.json')
 * 只在浏览器环境中使用
 */
...
let fetchPromise: Promise<PromptsData> | null = null
...
function normalizeData(raw: any): PromptsData {
  ...
  if (raw && raw.prompts && Array.isArray(raw.prompts)) {
    return raw as PromptsData
  }
...
async function getData(): Promise<PromptsData> {
  ...
  fetchPromise = fetch('/data/prompts.json')
    .then(res => {
    ...
    .catch(err => {
      console.error('[staticData-client] fetch failed:', err)
      fetchPromise = null
      return { meta: { total: 0, source: '', updated_at: '' }, categories: [], prompts: [] }
    })
...
export async function loadPromptsData(): Promise<PromptsData> { return getData() }
...
export async function loadRelated(slug: string, category: string, limit = 8): Promise<PromptCard[]> {
  try {
    const data = await getData()
    return data.prompts.filter(p => p && p.slug !== slug && p.category === category).slice(0, limit) as PromptCard[]
  } catch { return [] }
}
